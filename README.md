...
In every mode, it can be determined if LambdaSpeak 3 is ready to accept the next command or data byte, by reading a status byte from `&FBEE`. Mainly, there are **indicator bytes for READY and BUSY**; unfortunately, **different modes are using different READY and BUSY indicator bytes.** For exmaple, the Amstrad SSA-1 mode is using 128, the DKtronics is using 64, the native modes of LambdaSpeak (DECtalk and EPSON mode) are using 32, and 0 is used for BUSY. Moreover, in case data is communicated over the databus to the CPC (e.g., for commands that return the current voice, volume, received serial data in the serial interface mode, or data retrieved from the EEPROM), the READY / BUSY indicator bytes are only used partially in order to not interfere with the data being transmitted to the CPC. See the individual and more detailed mode descriptions for a in-depth explanations. 
